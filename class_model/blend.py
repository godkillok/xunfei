...
if __name__ == '__main__':
    ...
    for j, clf_process in enumerate(clfs):
        ...
        logging.info("dataset_blend_pred_j mean (sample,prob) {} ".format(np.mean(dataset_blend_pred_j,axis=3).shape))
        dataset_blend_pred[:, j,:] =np.mean(dataset_blend_pred_j,axis=3)
        dataset_blend_test[:, j,:] =np.mean(dataset_blend_test_j,axis=3)
